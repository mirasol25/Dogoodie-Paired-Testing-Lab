...
export default async function ReportsPage() {
  ...
  if (!study) return <div className="space-y-6"><PageHeader eyebrow="Study outputs" title="Reports & Evidence Package" description="Select an accessible study before generating reports." /></div>;
  ...
  return <ReportsClient study={study} protocol={getActiveProtocol(protocols)} assignments={assignments} pairs={pairs} reviews={reviews} validationResults={validationResults} evidence={evidence} activity={activity.events} activityTotal={activity.total} canExport={identity.profile.role !== "law_firm_viewer"} />;
}
