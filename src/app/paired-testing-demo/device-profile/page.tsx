...
export default async function DeviceProfilePage() {
  ...
  const { data, error } = await supabase.from("profiles").select("device_type,operating_system,operating_system_version,tester_country_name,tester_country_code").eq("id", identity.user.id).single();
  ...
  return <div className="space-y-7">
    ...
    <DeviceProfileForm initial={{ deviceType: data.device_type ?? "", operatingSystem: data.operating_system ?? "", operatingSystemVersion: data.operating_system_version ?? "" }} countryName={data.tester_country_name} countryCode={data.tester_country_code} />
  </div>;
}
