...
export default async function PairPage({ params }: { params: Promise<{ pairId: string }> }) {
  ...
  if (!pair) notFound();
  const [results, studyEvidence, reviews, serviceOptions] = await Promise.all([listPairValidationResults(pair.id), listStudyEvidence(study.id), listPairReviews(pair.id), listProviderServiceOptions()]);
