...
import { ArrowLeft, ArrowRight, BookOpenCheck, Clock3, GitCompareArrows, MapPin, ShieldCheck, Timer, type LucideIcon } from "lucide-react";
...
export default async function PairPage({ params }: { params: Promise<{ pairId: string }> }) {
  ...
  if (!pair) notFound();
  const [results, studyEvidence, reviews] = await Promise.all([listPairValidationResults(pair.id), listStudyEvidence(study.id), listPairReviews(pair.id)]);
...
function Metric({ label, value, icon: Icon, detail }: { label: string; value: string; icon: LucideIcon; detail?: string }) { return <div className="data-panel flex min-h-28 items-start justify-between rounded-md p-4"><div><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className="mt-2 text-xl font-semibold">{value}</p>{detail ? <p className="mt-1 text-[10px] text-muted-foreground">{detail}</p> : null}</div><Icon className="size-4 text-primary" /></div>; }
