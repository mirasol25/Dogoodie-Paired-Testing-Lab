...
import { ArrowLeft, ArrowRight, BookOpenCheck, Clock3, GitCompareArrows, MapPin, MessageSquareText, ShieldCheck, Timer, type LucideIcon } from "lucide-react";
...
export default async function PairPage({ params }: { params: Promise<{ pairId: string }> }) {
  ...
  if (!pair) notFound();
  const [results, studyEvidence, reviews, serviceOptions, ocrA, ocrB] = await Promise.all([listPairValidationResults(pair.id), listStudyEvidence(study.id), listPairReviews(pair.id), listProviderServiceOptions(), getSubmissionScreenshotValidation(pair.submissionA.id), getSubmissionScreenshotValidation(pair.submissionB.id)]);
