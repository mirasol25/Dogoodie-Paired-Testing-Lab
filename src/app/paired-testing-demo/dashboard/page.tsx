...
export default async function DashboardPage() {
  ...
  const [assignments, pairs, activity, serviceOptions] = await Promise.all([listStudyAssignments(study.id), listStudyMatchedPairs(study.id), listActivityLogFeed(study.id, { pageSize: 6 }), listProviderServiceOptions()]);
  const reviews = await listStudyReviews(pairs.map((pair) => pair.id));
  return <DashboardClient study={study} serviceOptions={serviceOptions} assignments={assignments} pairs={pairs} reviews={reviews} recentActivity={activity.events} />;
}
