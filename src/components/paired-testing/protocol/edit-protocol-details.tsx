...
export function EditProtocolDetails({ protocol }: { protocol: Protocol }) {
  const initial = { title: protocol.title, testerA: protocol.tester_a_value ?? "", testerB: protocol.tester_b_value ?? "" };
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await saveProtocolDetailsAction({ studyId: protocol.study_id, protocolId: protocol.id, title: values.title, testerAValue: values.testerA, testerBValue: values.testerB });
      if (result.ok) {
        setSaved(values);
