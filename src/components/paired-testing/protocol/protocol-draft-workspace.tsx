...
import { ProtocolDraftNavigationProvider } from "@/components/paired-testing/protocol/protocol-draft-navigation";
...
interface ProtocolDraftWorkspaceProps {
  ...
  discard: ReactNode;
  initialConfigureStep?: "details" | "conditions" | "thresholds" | "requirements" | "exclusions";
}

const persistentContent = "mt-5 data-[state=inactive]:hidden";

export function ProtocolDraftWorkspace({ version, details, conditions, thresholds, requirements, exclusions, comparison, preview, activation, discard, initialConfigureStep = "details" }: ProtocolDraftWorkspaceProps) {
  const [configureStep, setConfigureStep] = useState(initialConfigureStep);
  const [workspaceStep, setWorkspaceStep] = useState<"configure" | "changes" | "review">("configure");
  return (
    <ProtocolDraftNavigationProvider value={{ goToStep: setConfigureStep, goToWorkspace: setWorkspaceStep }}><section className="space-y-4 border-t border-border pt-6">
      ...
      <Tabs value={workspaceStep} onValueChange={(value) => setWorkspaceStep(value as typeof workspaceStep)} className="gap-0">
        ...
        <TabsContent value="configure" forceMount className={persistentContent}>
          <Tabs value={configureStep} onValueChange={(value) => setConfigureStep(value as typeof configureStep)} className="gap-0">
    ...
    </section></ProtocolDraftNavigationProvider>
  );
}
