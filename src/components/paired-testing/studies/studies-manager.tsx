...
export function CreateStudyForm({ providerOptions, initialData }: { providerOptions: ProviderServiceOption[]; initialData?: StudyEditorInitialData }) {
  ...
  return (
    <div className="relative space-y-7" aria-busy={pending}>
      {pending ? <div className="fixed inset-0 z-[80] grid place-items-center bg-background/75 px-4 backdrop-blur-sm" role="status" aria-live="polite">
        <div className="flex w-full max-w-sm items-center gap-4 rounded-md border border-primary/30 bg-card p-5 shadow-2xl">
          <span className="grid size-11 shrink-0 place-items-center rounded-md border border-primary/25 bg-primary/10"><LoaderCircle className="size-5 animate-spin text-primary" /></span>
          <div><p className="text-sm font-semibold">{initialData ? "Saving study changes" : "Creating study"}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{initialData ? "Updating the study configuration and initial route..." : "Creating the study, validating its route, and preparing the workspace..."}</p></div>
        </div>
      </div> : null}
      <div className="overflow-hidden rounded-md border border-border bg-secondary/10">
        <div className="grid grid-cols-2 sm:grid-cols-4">
        {stepLabels.map((label, index) => (
          <button key={label} type="button" onClick={() => goToStep(index)} disabled={pending} className={`min-h-20 border-b-2 px-3 py-3 text-left transition-colors disabled:cursor-wait sm:min-h-24 ${step === index ? "border-primary bg-primary/[0.06]" : "border-transparent hover:bg-secondary/50"}`} aria-current={step === index ? "step" : undefined}>
      ...
      <div className="sticky bottom-0 z-10 flex items-center justify-between border-t border-border bg-background/95 py-4 backdrop-blur">
        <Button type="button" variant="outline" onClick={() => goToStep(Math.max(0, step - 1))} disabled={step === 0 || pending}><ChevronLeft className="size-4" /><span className="sm:hidden">Back</span><span className="hidden sm:inline">{step === 0 ? "Back" : stepLabels[step - 1]}</span></Button>
        {step < 3 ? <Button type="button" onClick={() => goToStep(step + 1)} disabled={pending}><span className="sm:hidden">Continue</span><span className="hidden sm:inline">Continue to {stepLabels[step + 1]}</span><ChevronRight className="size-4" /></Button> : <Button type="button" onClick={submit} disabled={pending}>{pending ? <LoaderCircle className="size-4 animate-spin" /> : <Check className="size-4" />}{pending ? (initialData ? "Saving changes..." : "Creating study...") : (initialData ? "Save study changes" : "Create study")}</Button>}
      </div>
    </div>
  );
}
...
function CollectionNeed({ study, assignmentCount, readiness }: { study: Study; assignmentCount: number; readiness?: StudyCompletionReadiness }) {
  const target = study.target_pair_count ?? 0;
  const assignmentGap = target ? Math.max(target - assignmentCount, 0) : 0;
  const acceptedUsable = readiness?.pairs.accepted_usable ?? 0;
  const rejectedReviews = readiness?.reviews.rejected ?? 0;
  const replacementNeeded = target ? Math.min(rejectedReviews, Math.max(target - acceptedUsable, 0)) : rejectedReviews;
  const hasNeed = assignmentGap > 0 || replacementNeeded > 0;
  return <div className="min-w-44 text-xs"><p className={hasNeed ? "font-semibold text-amber-300" : "font-semibold text-primary"}>{hasNeed ? "Action needed" : "Target covered"}</p><p className="mt-1 text-muted-foreground">{target ? `${assignmentCount} / ${target} assignments scheduled` : `${assignmentCount} assignments scheduled`}</p>{assignmentGap > 0 ? <p className="mt-1 text-amber-300">{assignmentGap} more assignment{assignmentGap === 1 ? "" : "s"} needed</p> : null}{replacementNeeded > 0 ? <p className="mt-1 text-amber-300">{replacementNeeded} replacement pair{replacementNeeded === 1 ? "" : "s"} needed</p> : readiness ? <p className="mt-1 text-muted-foreground">{target ? `${acceptedUsable} / ${target} usable pairs` : `${acceptedUsable} usable pairs`}</p> : null}</div>;
}

function StudyList({ studies, activeProtocolStudyIds, canArchive, readiness, providerOptions, assignmentCounts }: { studies: Study[]; activeProtocolStudyIds: string[]; canArchive: boolean; readiness: Record<string, StudyCompletionReadiness>; providerOptions: ProviderServiceOption[]; assignmentCounts: Record<string, number> }) {
  const router = useRouter();
  ...
  const openingThisStudy = pendingId === study.id && isOpening;
  return <TableRow key={study.id}><TableCell className="min-w-72 whitespace-normal"><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => openStudy(study)} disabled={openingThisStudy} className="text-left font-medium hover:text-primary hover:underline disabled:cursor-wait">{study.name}</button></div><p className="mono mt-1 text-[10px] text-muted-foreground">{study.study_code}</p>{serviceLabel ? <p className="mt-2 text-xs font-medium text-primary">{serviceLabel}</p> : <p className="mt-2 text-xs text-muted-foreground">Testing service not configured</p>}</TableCell><TableCell className="text-xs">{study.study_type === "within_platform_pair" ? "Within platform" : "Cross platform"}</TableCell><TableCell><Badge variant={study.status === "active" ? "default" : "outline"} className="capitalize">{study.status}</Badge></TableCell><TableCell>{study.default_currency ?? "-"}</TableCell><TableCell className="text-xs text-muted-foreground">{new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(study.updated_at))}</TableCell><TableCell><CollectionNeed study={study} assignmentCount={assignmentCounts[study.id] ?? 0} readiness={readiness[study.id]} /></TableCell><TableCell className="min-w-64"><div className="space-y-2"><div className="flex justify-end gap-2"><Button asChild size="sm" variant="outline"><Link href={`/paired-testing-demo/studies/${study.id}/members`}><Users className="size-3.5" />Members</Link></Button><Button size="sm" onClick={() => openStudy(study)} disabled={openingThisStudy}>{openingThisStudy ? <LoaderCircle className="size-3.5 animate-spin" /> : <ArrowRight className="size-3.5" />}{openingThisStudy ? "Opening..." : "Open"}</Button></div>{canManageDraft || study.status !== "archived" ? <div className="flex flex-wrap items-center justify-end gap-1 border-t border-border pt-2"><span className="mr-auto text-[9px] uppercase text-muted-foreground">Manage</span>{canManageDraft ? <DraftStudyActions study={study} /> : null}<StudyLifecycleControl study={study} canArchive={canArchive} readiness={readiness[study.id]} /></div> : null}</div></TableCell></TableRow>;
  })}{!filteredStudies.length ? <TableRow><TableCell colSpan={7} className="h-28 text-center text-muted-foreground">No studies match the current filters.</TableCell></TableRow> : null}</TableBody></Table></div><p className="text-xs text-muted-foreground">Showing {filteredStudies.length} of {studies.length} studies</p></div>;
}

export function StudiesManager({ studies, activeProtocolStudyIds, providerOptions, role, readiness, assignmentCounts }: { studies: Study[]; activeStudyId: string | null; activeProtocolStudyIds: string[]; providerOptions: ProviderServiceOption[]; role: AppRole; readiness: Record<string, StudyCompletionReadiness>; assignmentCounts: Record<string, number> }) {
  const [tab, setTab] = useState("studies");
  return <Tabs value={tab} onValueChange={setTab}><div className="flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between"><div><TabsList><TabsTrigger value="studies">Study register</TabsTrigger><TabsTrigger value="create">Create study</TabsTrigger></TabsList><p className="mt-2 text-xs text-muted-foreground">Open a study to load its workspace, or manage its members and lifecycle.</p></div><Button onClick={() => setTab("create")}><Plus className="size-4" />New study</Button></div><TabsContent value="studies" className="mt-5"><StudyList studies={studies} activeProtocolStudyIds={activeProtocolStudyIds} canArchive={role === "admin"} readiness={readiness} providerOptions={providerOptions} assignmentCounts={assignmentCounts} /></TabsContent><TabsContent value="create" className="mt-5"><CreateStudyForm providerOptions={providerOptions} /></TabsContent></Tabs>;
}
