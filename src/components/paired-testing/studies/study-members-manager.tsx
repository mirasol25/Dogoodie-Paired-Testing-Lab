...
export function StudyMembersManager({ studyId, members, eligibleAccounts, canManageCoordinators, requiredTesterOs }: { studyId: string; members: StudyMember[]; eligibleAccounts: EligibleStudyAccount[]; canManageCoordinators: boolean; requiredTesterOs: string | null }) {
  ...
  return <div className="space-y-6"><div className="grid divide-y divide-border border-y border-border sm:grid-cols-4 sm:divide-x sm:divide-y-0">{[["Coordinators", counts.coordinators], ["Testers", counts.testers], ["Reviewers", counts.reviewers], ["Viewers", counts.viewers]].map(([label, count]) => <div key={label} className="py-4 sm:px-4 sm:first:pl-0"><p className="text-[10px] text-muted-foreground">{label}</p><p className="mt-1 text-lg font-semibold">{count}</p></div>)}</div>
    {requiredTesterOs ? <div className="rounded-md border border-primary/30 bg-primary/5 px-4 py-3 text-xs"><span className="font-semibold">Tester OS requirement:</span> Only active testers with {requiredTesterOs} in their device profile are available to add.</div> : null}
    <div className="rounded-md border border-border bg-secondary/20 px-4 py-3 text-xs leading-5 text-muted-foreground"><span className="font-semibold text-foreground">Viewer release access:</span> Add each viewer here as an active member. Their read-only study access begins when the study is completed or archived.</div>
