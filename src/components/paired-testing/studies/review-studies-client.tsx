...
import { ArrowRight, Check, CheckCircle2, CircleDashed, Flag, GitCompareArrows, LoaderCircle, Search, XCircle } from "lucide-react";
...
export function ReviewStudiesClient({ rows, activeStudyId }: { rows: Array<{ study: Study; workload: ReviewerStudyWorkload }>; activeStudyId: string | null }) {
  ...
  const visible = useMemo(() => rows.filter(({ study }) => status === "all" || study.status === status).filter(({ study }) => `${study.study_code} ${study.name}`.toLowerCase().includes(query.trim().toLowerCase())).sort((a, b) => b.workload.pending - a.workload.pending || b.workload.flagged - a.workload.flagged || a.study.name.localeCompare(b.study.name)), [query, rows, status]);
  ...
  const totalFlagged = rows.reduce((sum, row) => sum + row.workload.flagged, 0);
  ...
  return <div className="space-y-4"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Pending review" value={totalPending} icon={CircleDashed} /><Metric label="Flagged follow-up" value={totalFlagged} icon={Flag} /><Metric label="Accepted" value={totalAccepted} icon={CheckCircle2} /><Metric label="Rejected" value={totalRejected} icon={XCircle} /><Metric label="Assigned pairs" value={totalPairs} icon={GitCompareArrows} /></div><section className="overflow-hidden rounded-md border border-border"><div className="grid gap-2 border-b border-border bg-card/35 p-3 sm:grid-cols-[minmax(0,1fr)_170px]"><div className="relative max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search study name or code" className="pl-9" /></div><Select value={status} onValueChange={setStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All statuses</SelectItem><SelectItem value="draft">Draft</SelectItem><SelectItem value="active">Active</SelectItem><SelectItem value="paused">Paused</SelectItem><SelectItem value="completed">Completed</SelectItem><SelectItem value="archived">Archived</SelectItem></SelectContent></Select></div><div className="divide-y divide-border">{visible.map(({ study, workload }) => { const selected = study.id === activeStudyId; const date = (value: string | null) => value ? new Intl.DateTimeFormat("en", { dateStyle: "medium", timeZone: study.display_timezone }).format(new Date(value)) : "Open"; return <article key={study.id} className={selected ? "bg-primary/[0.04]" : undefined}><div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(240px,1.3fr)_minmax(210px,1fr)_minmax(340px,1.2fr)_auto] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-sm font-semibold">{study.name}</h2>{selected ? <span className="flex items-center gap-1 text-[10px] text-primary"><Check className="size-3" />Selected</span> : null}</div><p className="mono mt-1 text-[10px] text-muted-foreground">{study.study_code}</p><p className="mt-2 text-xs capitalize text-muted-foreground">{study.status} | {study.default_currency ?? "Currency pending"}</p></div><div className="text-xs"><p className="font-medium">{study.display_timezone}</p><p className="mt-1 text-muted-foreground">{date(study.testing_starts_at)} - {date(study.testing_ends_at)}</p></div><div className="grid grid-cols-5 gap-2 text-center"><Count label="Pending" value={workload.pending} tone={workload.pending ? "primary" : "muted"} /><Count label="Flagged" value={workload.flagged} tone={workload.flagged ? "warning" : "muted"} /><Count label="Accepted" value={workload.accepted} tone={workload.accepted ? "primary" : "muted"} /><Count label="Rejected" value={workload.rejected} tone={workload.rejected ? "danger" : "muted"} /><Count label="Total" value={workload.total} tone="muted" /></div><Button onClick={() => open(study)} disabled={pending} variant={workload.pending || selected ? "default" : "outline"}>{pendingId === study.id ? <LoaderCircle className="size-4 animate-spin" /> : workload.pending ? "Review queue" : "Open study"}<ArrowRight className="size-4" /></Button></div></article>; })}{!visible.length ? <div className="px-6 py-14 text-center"><p className="text-sm font-medium">No assigned studies match</p><p className="mt-1 text-xs text-muted-foreground">Adjust the search or study-status filter.</p></div> : null}</div><div className="border-t border-border px-4 py-2 text-[10px] text-muted-foreground">{visible.length} of {rows.length} assigned studies</div></section></div>;
}

function Metric({ label, value, icon: Icon }: { label: string; value: number; icon: typeof Flag }) { return <div className="data-panel flex min-h-24 items-center justify-between rounded-md p-4"><div><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div><Icon className="size-5 text-primary" /></div>; }
