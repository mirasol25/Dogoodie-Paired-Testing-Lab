...
import { createAssignmentAction } from "@/app/paired-testing-demo/assignments/actions";
...
export function AssignmentsClient({ study, assignments, setupOptions, testerOptions, canManage }: { study: Study; assignments: AssignmentSummary[]; setupOptions: AssignmentSetupOptions; testerOptions: AssignmentTesterOption[]; canManage: boolean }) {
...
function AssignmentSetupDialog({ study, options, testers }: { study: Study; options: AssignmentSetupOptions; testers: AssignmentTesterOption[] }) {
  ...
  const [testerAId, setTesterAId] = useState("");
  const [testerBId, setTesterBId] = useState("");
  ...
  function continueToReview() {
    const parsed = assignmentTesterPairSchema.safeParse({ testerAId, testerBId });
    const nextErrors: Record<string, string> = {};
    if (!parsed.success) parsed.error.issues.forEach((issue) => { nextErrors[String(issue.path[0])] ??= issue.message; });
    setErrors(nextErrors);
  ...
  function submitAssignment() {
    startTransition(async () => {
      const result = await createAssignmentAction({ studyId: study.id, protocolId, routeId, testerAServiceId, testerBServiceId: isCrossPlatform ? testerBServiceId : testerAServiceId, testingDate, startTime, endTime, testerAId, testerBId, timezone, instructions });
      if (!result.ok) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogTrigger asChild><Button><CalendarPlus className="size-4" />Create assignment</Button></DialogTrigger>
    <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
      ...
      {!setupReady ? <div className="border-y border-border py-6 text-sm"><p className="font-medium">Assignment setup is not ready.</p><p className="mt-1 text-xs leading-5 text-muted-foreground">This study needs an active protocol, an active route, configured provider ride tiers, and at least two active Tester members.</p></div> : <div className="space-y-6">
        ...
        <section className="space-y-4 rounded-md border border-primary/25 bg-primary/[0.025] p-4"><div className="flex flex-wrap items-end justify-between gap-2"><div><p className="text-sm font-semibold">Testing window</p><p className="mt-1 text-xs text-muted-foreground">One synchronized session for both testers</p></div><div className="text-right"><p className="mono text-[10px] font-medium text-primary">{timezone}</p><p className="mt-1 text-[10px] text-muted-foreground">{currentStudyDateTime.replace("T", " ")}</p></div></div>
        ...
        </> : step === 1 ? <TesterPairStep protocol={options.protocols.find((protocol) => protocol.id === protocolId)} serviceA={options.services.find((service) => service.id === testerAServiceId)} serviceB={options.services.find((service) => service.id === (isCrossPlatform ? testerBServiceId : testerAServiceId))} testers={testers} testerAId={testerAId} testerBId={testerBId} errors={errors} onTesterAChange={(value) => { setTesterAId(value); if (value === testerBId) setTesterBId(""); setErrors({}); }} onTesterBChange={(value) => { setTesterBId(value); setErrors({}); }} onBack={() => { setStep(0); setErrors({}); }} onContinue={continueToReview} /> : <AssignmentReview study={study} protocol={options.protocols.find((protocol) => protocol.id === protocolId)} route={selectedRoute} serviceA={options.services.find((service) => service.id === testerAServiceId)} serviceB={options.services.find((service) => service.id === (isCrossPlatform ? testerBServiceId : testerAServiceId))} testerA={testers.find((tester) => tester.id === testerAId)} testerB={testers.find((tester) => tester.id === testerBId)} testingDate={testingDate} startTime={startTime} endTime={endTime} timezone={timezone} instructions={instructions} pending={pending} onInstructionsChange={setInstructions} onBack={() => setStep(1)} onCreate={submitAssignment} />}
      </div>}
    </DialogContent>
  </Dialog>;
}

function TesterPairStep({ protocol, serviceA, serviceB, testers, testerAId, testerBId, errors, onTesterAChange, onTesterBChange, onBack, onContinue }: { protocol?: AssignmentSetupOptions["protocols"][number]; serviceA?: AssignmentSetupOptions["services"][number]; serviceB?: AssignmentSetupOptions["services"][number]; testers: AssignmentTesterOption[]; testerAId: string; testerBId: string; errors: Record<string, string>; onTesterAChange: (value: string) => void; onTesterBChange: (value: string) => void; onBack: () => void; onContinue: () => void }) {
  const testerA = testers.find((tester) => tester.id === testerAId);
  const testerB = testers.find((tester) => tester.id === testerBId);
  return <div className="space-y-6">
    <div><p className="text-sm font-semibold">Assign tester sides</p><p className="mt-1 text-xs text-muted-foreground">Choose a different active tester for each side.</p></div>
    <div className="grid gap-5 sm:grid-cols-2">
      <TesterSide side="Tester A" condition={protocol?.testerAValue ?? "Not configured"} variable={protocol?.isolatedVariable ?? "Protocol condition"} service={serviceA ? `${serviceA.platformName} - ${serviceA.serviceName}` : "Not selected"} selectedId={testerAId} testers={testers.filter((tester) => tester.id !== testerBId)} error={errors.testerAId} onChange={onTesterAChange} />
      <TesterSide side="Tester B" condition={protocol?.testerBValue ?? "Not configured"} variable={protocol?.isolatedVariable ?? "Protocol condition"} service={serviceB ? `${serviceB.platformName} - ${serviceB.serviceName}` : "Not selected"} selectedId={testerBId} testers={testers.filter((tester) => tester.id !== testerAId)} error={errors.testerBId} onChange={onTesterBChange} />
    </div>
    <div className={`border-l-2 pl-3 text-xs ${testerA && testerB ? "border-primary text-foreground" : "border-border text-muted-foreground"}`}>{testerA && testerB ? `${testerA.displayName} and ${testerB.displayName} are ready to be paired.` : "Select a different account for each tester side."}</div>
...
function AssignmentReview({ study, protocol, route, serviceA, serviceB, testerA, testerB, testingDate, startTime, endTime, timezone, instructions, pending, onInstructionsChange, onBack, onCreate }: { study: Study; protocol?: AssignmentSetupOptions["protocols"][number]; route?: AssignmentSetupOptions["routes"][number]; serviceA?: AssignmentSetupOptions["services"][number]; serviceB?: AssignmentSetupOptions["services"][number]; testerA?: AssignmentTesterOption; testerB?: AssignmentTesterOption; testingDate: string; startTime: string; endTime: string; timezone: string; instructions: string; pending: boolean; onInstructionsChange: (value: string) => void; onBack: () => void; onCreate: () => void }) {
  ...
  return <div className="space-y-6">
    <div><p className="text-sm font-semibold">Assignment summary</p><p className="mt-1 text-xs text-muted-foreground">Review the controlled session before creating it.</p></div>
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <ReviewTester side="Tester A" tester={testerA} service={serviceA} variable={protocol?.isolatedVariable} condition={protocol?.testerAValue} />
      <ReviewTester side="Tester B" tester={testerB} service={serviceB} variable={protocol?.isolatedVariable} condition={protocol?.testerBValue} />
    </div>
    ...
    <div className="flex items-center justify-between border-t border-border pt-4"><Button variant="outline" onClick={onBack} disabled={pending}>Back</Button><div className="flex items-center gap-3"><p className="hidden text-xs text-muted-foreground sm:block">Creates one assignment and two tester slots</p><Button onClick={onCreate} disabled={pending}>{pending ? "Creating..." : "Create assignment"}</Button></div></div>
  </div>;
}
