...
function AssignmentSetupDialog({ study, options, testers, capacity }: { study: Study; options: AssignmentSetupOptions; testers: AssignmentTesterOption[]; capacity: StudyCollectionCapacity }) {
  ...
  const availablePairCount = testerAOperatingSystem && testerBOperatingSystem && testerAOperatingSystem !== testerBOperatingSystem
    ? Math.min(testerAOptions.length, testerBOptions.length)
    : Math.floor(testerAOptions.length / 2);
  const maximumPairs = Math.max(1, Math.min(availablePairCount, capacity.target === null ? availablePairCount : capacity.assignmentsNeeded));
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
      ...
      {!setupReady ? <div className="border-y border-border py-6 text-sm"><p className="font-medium">Assignment setup is not ready.</p><p className="mt-1 text-xs leading-5 text-muted-foreground">This study needs an active protocol, route, provider tiers, and eligible Tester members whose saved operating systems match both study sides.</p></div> : <div className="space-y-6">
        ...
        <div className="grid overflow-hidden rounded-md border border-border sm:grid-cols-2 sm:divide-x sm:divide-border">
          <LockedField label="Tester A device condition" value={testerAOperatingSystem ?? "Not controlled"} detail="Verified from the tester device profile" />
          <LockedField label="Tester B device condition" value={testerBOperatingSystem ?? "Not controlled"} detail={testerAOperatingSystem !== testerBOperatingSystem ? "Intentional operating-system difference" : "Same operating-system control"} />
        </div>
        ...
        {asynchronousTesting ? <div className="space-y-4">
          <div className="grid gap-4 border-b border-border pb-4 sm:grid-cols-2"><DateTimeField type="date" label="Shared testing date" value={testingDate} onChange={(value) => { setTestingDate(value); setErrors({}); }} min={minimumDate} max={maximumDate || undefined} error={errors.testingDate} /><div className="space-y-2"><div className="flex items-center gap-2"><Label htmlFor="assignment-pair-count">Paired sessions</Label><ProtocolInfoTooltip label="Paired sessions info">You can create up to {maximumPairs} paired sessions because each session needs a distinct tester pair. If you need more than {maximumPairs} pairs, create another assignment batch.</ProtocolInfoTooltip></div><Input id="assignment-pair-count" type="number" min={1} max={maximumPairs} value={testerPairs.length} onChange={(event) => setPairCount(Number(event.target.value))} /><p className="text-[10px] text-muted-foreground">Up to {maximumPairs} using distinct testers.</p></div></div>
...
function TesterPairStep({ protocol, serviceA, serviceB, testers, testerAOptions, testerBOptions, testerAOperatingSystem, testerBOperatingSystem, pairs, errors, onChange, onBack, onContinue }: { protocol?: AssignmentSetupOptions["protocols"][number]; serviceA?: AssignmentSetupOptions["services"][number]; serviceB?: AssignmentSetupOptions["services"][number]; testers: AssignmentTesterOption[]; testerAOptions: AssignmentTesterOption[]; testerBOptions: AssignmentTesterOption[]; testerAOperatingSystem: string | null; testerBOperatingSystem: string | null; pairs: Array<{ testerAId: string; testerBId: string }>; errors: Record<string, string>; onChange: (index: number, side: "testerAId" | "testerBId", value: string) => void; onBack: () => void; onContinue: () => void }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="space-y-4">{pairs.map((pair, index) => {
      ...
      return <section key={index} className="overflow-hidden rounded-md border border-border"><div className="flex items-center justify-between border-b border-border bg-secondary/20 px-4 py-3"><p className="text-sm font-semibold">Pair {index + 1}</p><p className="text-[10px] uppercase text-muted-foreground">{testerA && testerB ? "Ready" : "Needs testers"}</p></div><div className="grid gap-4 p-4 sm:grid-cols-2">
        <TesterSide side="Tester A" condition={protocol?.testerAValue ?? "Not configured"} variable={protocol?.isolatedVariable ?? "Protocol condition"} service={serviceA ? `${serviceA.platformName} - ${serviceA.serviceName}` : "Not selected"} expectedOperatingSystem={testerAOperatingSystem} selectedId={pair.testerAId} testers={availableFor(pair.testerAId).filter((tester) => testerAOptions.some((option) => option.id === tester.id) && tester.id !== pair.testerBId)} error={errors[`pairs.${index}.testerAId`]} onChange={(value) => onChange(index, "testerAId", value)} />
        <TesterSide side="Tester B" condition={protocol?.testerBValue ?? "Not configured"} variable={protocol?.isolatedVariable ?? "Protocol condition"} service={serviceB ? `${serviceB.platformName} - ${serviceB.serviceName}` : "Not selected"} expectedOperatingSystem={testerBOperatingSystem} selectedId={pair.testerBId} testers={availableFor(pair.testerBId).filter((tester) => testerBOptions.some((option) => option.id === tester.id) && tester.id !== pair.testerAId)} error={errors[`pairs.${index}.testerBId`]} onChange={(value) => onChange(index, "testerBId", value)} />
      </div></section>;
    })}</div>
...
function TesterSide({ side, condition, variable, service, expectedOperatingSystem, selectedId, testers, error, onChange }: { side: string; condition: string; variable: string; service: string; expectedOperatingSystem: string | null; selectedId: string; testers: AssignmentTesterOption[]; error?: string; onChange: (value: string) => void }) {
