...
export function AssignmentDetails({ study, assignment, routeGuidance, submission, technicalProfile, evidence, currentUserId, canManage, operations }: { study: Study; assignment: AssignmentSummary; routeGuidance: AssignmentRouteGuidance | null; submission: SubmissionRow | null; technicalProfile: Pick<SubmissionRow, "device_type" | "operating_system" | "operating_system_version"> | null; evidence: EvidenceRow[]; currentUserId: string; canManage: boolean; operations: AssignmentOperationalSummary | null }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
      <div className="space-y-5"><div><p className="text-[10px] uppercase text-muted-foreground">Route</p><h2 className="mt-1.5 text-base font-semibold">{assignment.pickup_location} to {assignment.destination_location}</h2></div><div className="grid overflow-hidden rounded-md border border-border sm:grid-cols-2 sm:divide-x sm:divide-border"><Location label="Pickup" value={assignment.pickup_location} instructions={routeGuidance?.pickupInstructions ?? null} accent="primary" /><Location label="Destination" value={assignment.destination_location} instructions={routeGuidance?.destinationInstructions ?? null} accent="amber" /></div></div>
      <div className="space-y-4"><div><p className="text-[10px] uppercase text-muted-foreground">Testing window</p><h2 className="mt-1.5 text-base font-semibold">{asynchronousTesting ? "Tester-side schedule" : "Scheduled paired session"}</h2></div><div className="rounded-md border border-primary/25 bg-primary/[0.025] p-4"><div className="flex gap-3"><CalendarClock className="mt-0.5 size-4 shrink-0 text-primary" /><div><p className="text-sm font-medium">{ownTester ? `${ownSlot === "tester_a" ? "Tester A" : "Tester B"}: ` : ""}{formatSchedule(ownTester?.scheduledStart ?? assignment.scheduled_start, timezone)}</p><p className="mt-1 text-sm text-muted-foreground">until {formatSchedule(ownTester?.scheduledEnd ?? assignment.scheduled_end, timezone)}</p><p className="mono mt-3 text-[10px] text-primary">{timezone}</p>{asynchronousTesting ? <p className="mt-2 text-xs text-muted-foreground">Separate tester windows apply. Request-time synchronization is not evaluated.</p> : null}</div></div></div></div>
    </section>
...
function Location({ label, value, instructions, accent }: { label: string; value: string; instructions: string | null; accent: "primary" | "amber" }) {
  return <div className="flex gap-3 p-4"><MapPin className={`mt-0.5 size-4 shrink-0 ${accent === "primary" ? "text-primary" : "text-amber-400"}`} /><div className="min-w-0"><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className="mt-1 text-sm font-medium">{value}</p><div className="mt-3 border-t border-border pt-3"><p className="text-[10px] uppercase text-muted-foreground">{label} instructions</p><p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-foreground">{instructions || `No specific ${label.toLowerCase()} instructions were added.`}</p></div></div></div>;
}
