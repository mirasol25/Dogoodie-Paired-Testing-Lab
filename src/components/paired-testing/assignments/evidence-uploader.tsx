...
const screenshotGuides: Record<string, { image: string; alt: string; title: string }> = {
  grab: { image: "/screenshot-guides/grab-selected-ride.png", alt: "Example Grab quote screenshot with selected ride, fare, time, and battery visible.", title: "Grab quote example" },
  uber: { image: "/screenshot-guides/uber-selected-ride.jpg", alt: "Example Uber quote screenshot with selected ride, fare, time, and battery visible.", title: "Uber quote example" },
};

function ScreenshotCaptureGuide({ platformName, serviceName }: { platformName: string | null; serviceName: string | null }) {
  const guide = screenshotGuides[platformName?.toLocaleLowerCase() ?? ""];
  return <aside className="rounded-md border border-primary/25 bg-primary/[0.035] p-3"><div className="flex gap-3">{guide ? <a href={guide.image} target="_blank" rel="noreferrer" className="relative hidden h-40 w-20 shrink-0 overflow-hidden rounded border border-border bg-background sm:block" aria-label={`Open full ${guide.title.toLowerCase()}`}><Image src={guide.image} alt="" fill sizes="80px" className="object-cover object-bottom" /></a> : null}<div className="min-w-0"><div className="flex items-center gap-2"><CircleAlert className="size-4 text-primary" /><p className="text-xs font-semibold">{guide?.title ?? "Quote screenshot checklist"}</p></div><p className="mt-1 text-xs leading-5 text-muted-foreground">Capture the whole phone screen - do not crop, blur, or cover the status bar. Expected service: <span className="font-medium text-foreground">{serviceName ?? "assigned ride"}</span>.</p><ul className="mt-2 space-y-1 text-xs text-muted-foreground"><li>- Make the selected ride card and its fare visible.</li><li>- Keep the status-bar time visible at the top left.</li><li>- Keep the battery percentage visible at the top right.</li><li>- If OCR leaves time or battery blank, enter it manually before saving.</li></ul>{guide ? <a href={guide.image} target="_blank" rel="noreferrer" className="mt-2 inline-block text-[11px] font-medium text-primary underline-offset-2 hover:underline">Open full example</a> : null}</div></div></aside>;
}
...
export function EvidenceUploader({ assignment, ownSlot, submissionId, observationSaved, initialEvidence, onOCRResult, onEvidenceStateChange, showFinalSubmission = true }: { assignment: AssignmentSummary; ownSlot: AssignmentTesterSummary; submissionId: string | null; observationSaved: boolean; initialEvidence: EvidenceRow[]; onOCRResult?: (result: ScreenshotValidationResult) => void; onEvidenceStateChange?: (state: { requiredComplete: boolean; mismatched: boolean }) => void; showFinalSubmission?: boolean }) {
  ...
  async function upload(requirement: Requirement) {
    ...
    if (!activeSubmissionId && requirement.code === "screenshot") {
      ...
      activeSubmissionId = draft.submissionId;
      setDraftId(activeSubmissionId);
    }
    ...
    try {
      ...
      if (requirement.code === "screenshot" && result.evidenceId) {
        ...
        else { setValidation(ocr.validation); onOCRResult?.(ocr.validation); toast.success(ocr.message); }
      }
  ...
  function submit() {
    if (!submissionId || !observationSaved || !requiredComplete || validation?.serviceValidation === "mismatched") return;
  ...
  return <section className="space-y-4 border-t border-border pt-5"><div><p className="text-[10px] uppercase text-muted-foreground">1. Quote screenshot</p><h2 className="mt-1.5 text-base font-semibold">Upload and validate the selected service</h2><p className="mt-1 text-xs text-muted-foreground">The first screenshot automatically creates a draft and verifies the assignment service.</p></div><ScreenshotCaptureGuide platformName={ownSlot.platformName} serviceName={ownSlot.serviceName} />{validation ? <div className={`rounded-md border p-3 text-xs ${validation.serviceValidation === "matched" ? "border-primary/35 bg-primary/[0.05]" : validation.serviceValidation === "mismatched" ? "border-red-400/40 bg-red-400/[0.06]" : "border-amber-400/40 bg-amber-400/[0.06]"}`}><p className="font-medium">{validation.serviceValidation === "matched" ? "Required service verified" : validation.serviceValidation === "mismatched" ? "Selected service mismatch" : "Service could not be automatically verified"}</p><p className="mt-1 text-muted-foreground">Required: {ownSlot.serviceName ?? "Assigned service"} Â· Detected: {validation.selectedRideLabel ?? "Unreadable"}</p>{validation.serviceValidation === "mismatched" ? <p className="mt-1 text-red-300">Upload a replacement screenshot before normal submission.</p> : null}</div> : null}<div className="divide-y divide-border overflow-hidden rounded-md border border-border">{requirements.map((requirement) => {
    const Icon = icons[requirement.code];
    const uploaded = requirement.code !== "screenshot" && uploadedTypes.includes(requirement.code);
    return <div key={requirement.code} className="grid gap-3 p-4 sm:grid-cols-[1fr_minmax(220px,1fr)_auto] sm:items-center"><div className="flex items-center gap-3"><Icon className="size-4 text-primary" /><div><p className="text-sm font-medium">{requirement.label}</p><Badge variant={requirement.required ? "outline" : "secondary"} className="mt-1">{requirement.required ? "Required" : "Configured"}</Badge></div></div>{uploaded ? <p className="text-xs font-medium text-primary">Uploaded</p> : <Input type="file" accept={accepts[requirement.code]} disabled={(requirement.code !== "screenshot" && !activeSubmissionId) || uploading !== null} onChange={(event) => setFiles((current) => ({ ...current, [requirement.code]: event.target.files?.[0] }))} />}<Button size="sm" variant={uploaded ? "outline" : "default"} disabled={uploaded || !files[requirement.code] || uploading !== null || (requirement.code !== "screenshot" && !activeSubmissionId)} onClick={() => void upload(requirement)}>{uploading === requirement.code ? <LoaderCircle className="size-4 animate-spin" /> : <Upload className="size-4" />}{uploaded ? "Complete" : uploading === requirement.code ? "Uploading..." : requirement.code === "screenshot" && uploadedTypes.includes("screenshot") ? "Replace" : "Upload"}</Button></div>;
  })}<div className="grid gap-3 bg-card/20 p-4 sm:grid-cols-[1fr_minmax(220px,1fr)_auto] sm:items-center"><div className="flex items-center gap-3"><FileJson className="size-4 text-primary" /><div><p className="text-sm font-medium">System-generated metadata</p><Badge variant="outline" className="mt-1">Required</Badge></div></div><p className="text-xs text-muted-foreground">Filename, type, size, hash, timestamps, uploader, and storage linkage are captured automatically.</p><p className={`text-xs font-medium ${requiredComplete ? "text-primary" : "text-muted-foreground"}`}>{requiredComplete ? "Complete" : "Generated with uploads"}</p></div></div>{showFinalSubmission ? <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4"><div><p className="text-xs font-medium">Final submission</p><p className="mt-1 text-xs text-muted-foreground">{validation?.serviceValidation === "mismatched" ? "A replacement screenshot is required." : !observationSaved ? "Save the latest observation changes first." : !requiredComplete ? "Upload all required evidence to continue." : "Observation and required evidence are complete."}</p></div><Button onClick={submit} disabled={!submissionId || !observationSaved || !requiredComplete || uploading !== null || submitting || validation?.serviceValidation === "mismatched"}>{submitting ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />}{submitting ? "Submitting..." : "Submit observation"}</Button></div> : null}</section>;
}
