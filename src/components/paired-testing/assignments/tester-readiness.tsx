...
export function TesterReadiness({ assignment, ownSlot, partnerSlot }: { assignment: AssignmentSummary; ownSlot: AssignmentTesterSummary; partnerSlot?: AssignmentTesterSummary }) {
  ...
  return <section className="space-y-4 border-t border-border pt-5"><div><p className="text-[10px] uppercase text-muted-foreground">Tester readiness</p><h2 className="mt-1.5 text-base font-semibold">Confirm your assignment</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Check the assigned test conditions only. You will prepare your screenshot and screen-recording tools immediately before starting the test.</p></div><div className="divide-y divide-border overflow-hidden rounded-md border border-border">{checklist.map((item) => <label key={item} className="flex min-h-12 cursor-pointer items-start gap-3 px-4 py-3 text-xs leading-5 hover:bg-secondary/30"><Checkbox className="mt-0.5" checked={checked.includes(item)} onCheckedChange={(value) => setChecked((current) => value === true ? [...current, item] : current.filter((entry) => entry !== item))} /><span>{item}</span></label>)}</div><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted-foreground">{checked.length} of {checklist.length} assignment details confirmed</p><Button onClick={confirm} disabled={!complete || pending}>{pending ? <LoaderCircle className="size-4 animate-spin" /> : null}{pending ? "Confirming..." : "Confirm ready"}</Button></div></section>;
}
