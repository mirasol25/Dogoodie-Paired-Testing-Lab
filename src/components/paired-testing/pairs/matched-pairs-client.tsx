...
import { ArrowRight, CheckCircle2, CircleDashed, Flag, GitCompareArrows, Search, XCircle } from "lucide-react";
...
export function MatchedPairsClient({ pairs, timezone, canReview }: { pairs: MatchedPairSummary[]; timezone: string; canReview: boolean }) {
  ...
  const visible = useMemo(() => pairs.filter((pair) => {
    ...
    if (reviewStatus !== "all" && pair.reviewStatus !== reviewStatus) return false;
  ...
  const reviewCount = (value: MatchedPairSummary["reviewStatus"]) => pairs.filter((pair) => pair.reviewStatus === value).length;
  ...
  return <div className="space-y-4">
    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Summary label="Flagged" value={reviewCount("flagged")} icon={Flag} tone="warning" />
    ...
    <div className="overflow-hidden rounded-md border border-border">
      <div className="flex flex-col gap-2 border-b border-border bg-card/35 p-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><Select value={reviewStatus} onValueChange={setReviewStatus}><SelectTrigger className="h-9 w-full bg-background/45 text-xs sm:w-40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All review statuses</SelectItem>{["pending", "accepted", "flagged", "rejected"].map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select><Select value={technicalStatus} onValueChange={setTechnicalStatus}><SelectTrigger className="h-9 w-full bg-background/45 text-xs sm:w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All technical statuses</SelectItem>{["pending", "valid", "warning", "invalid", "incomplete"].map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div>
      </div>
