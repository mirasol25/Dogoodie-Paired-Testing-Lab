...
export function PairEvidenceView({ records, submissionAId, submissionBId, timezone, canOpenFiles }: { records: EvidenceRecord[]; submissionAId: string; submissionBId: string; timezone: string; canOpenFiles: boolean }) {
  return <div className="grid gap-4 lg:grid-cols-2"><TesterEvidence label="Tester A evidence" files={records.filter((item) => item.submission_id === submissionAId)} timezone={timezone} canOpenFiles={canOpenFiles} /><TesterEvidence label="Tester B evidence" files={records.filter((item) => item.submission_id === submissionBId)} timezone={timezone} canOpenFiles={canOpenFiles} /></div>;
}

function TesterEvidence({ label, files, timezone, canOpenFiles }: { label: string; files: EvidenceRecord[]; timezone: string; canOpenFiles: boolean }) {
