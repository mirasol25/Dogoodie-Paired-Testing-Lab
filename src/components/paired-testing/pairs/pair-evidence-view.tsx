...
function missingMetadata(files: Array<EvidenceRecord | undefined>) {
  const missing = new Set<string>();
  if (files.some((file) => !file)) missing.add("required file");
  for (const file of files.filter((item): item is EvidenceRecord => Boolean(item))) {
    if (!file.original_filename) missing.add("filename");
    if (!file.mime_type) missing.add("MIME type");
    if (file.size_bytes <= 0) missing.add("file size");
    if (!file.sha256) missing.add("fingerprint");
    if (!file.captured_at || !file.uploaded_at) missing.add("timestamps");
    if (!file.uploaded_by) missing.add("uploader");
    if (!file.storage_bucket || !file.storage_path) missing.add("storage linkage");
  }
  return [...missing];
}
