...
returns jsonb language sql stable security definer set search_path = '' as $$
  with study_record as (
    ...
    select mp.*, coalesce(latest.status, 'pending'::public.review_status) review_status,
      coalesce(latest.technical_exception, false) technical_exception
    from public.matched_pairs mp
    left join lateral (
      select er.status, er.technical_exception from public.expert_reviews er
    ...
    select count(*)::int total,
      ...
      count(*) filter (where review_status = 'accepted' and technical_exception)::int accepted_with_exception,
      ...
      count(*) filter (
        where review_status = 'accepted'
          ...
          and (technical_status in ('valid', 'warning') or technical_exception)
      )::int accepted_usable
    from pair_rows
  ), missing_pairs as (
  ...
  select jsonb_build_object(
    ...
    'reviews', jsonb_build_object('pending', pc.pending_review, 'accepted', pc.accepted, 'accepted_with_exception', pc.accepted_with_exception, 'flagged', pc.flagged, 'rejected', pc.rejected),
...
comment on function private.study_completion_readiness(uuid) is
  'Counts accepted technical exceptions toward the target only when their required evidence is complete, while retaining the exception in the review record and audit log.';
