drop function if exists public.save_expert_review(uuid, public.review_status, text);

create or replace function public.save_expert_review(p_matched_pair_id uuid, p_status public.review_status, p_reason text, p_note text)
...
begin
  ...
  insert into public.expert_reviews (matched_pair_id, reviewer_id, status, reason, note, decided_at)
  ...
  values (selected_pair.study_id, caller_id,
    case when p_status = 'pending' then 'review.cleared' else 'review.' || p_status::text end,
    'review', 'pair', selected_pair.id,
    jsonb_build_object('pair_code', selected_pair.pair_code, 'status', p_status, 'previous_status', previous_status, 'reason', saved_review.reason, 'note', saved_review.note));
  return saved_review;
end;
$$;

revoke all on function public.save_expert_review(uuid, public.review_status, text, text) from public;
grant execute on function public.save_expert_review(uuid, public.review_status, text, text) to authenticated;
