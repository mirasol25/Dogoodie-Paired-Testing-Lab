...
begin
  ...
  if p_status = 'flagged' then raise exception 'Flagged review decisions are no longer supported. Accept, accept with a technical exception, or reject the pair.' using errcode = '22023'; end if;
  ...
  if not exists (
    ...
    join public.study_members sm on sm.user_id = p.id and sm.study_id = selected_pair.study_id and sm.study_role = 'expert_reviewer' and sm.membership_status = 'active'
  ...
  if p_status = 'accepted' and selected_pair.technical_status = 'invalid' and not p_technical_exception then raise exception 'An invalid pair requires a documented technical exception or rejection' using errcode = '22023'; end if;
  if p_status = 'rejected' and length(trim(coalesce(p_note, ''))) < 10 then raise exception 'Rejected decisions require a reviewer note' using errcode = '22023'; end if;
  ...
  on conflict (matched_pair_id, reviewer_id) do update set
    status = excluded.status, reason = excluded.reason, note = excluded.note, technical_exception = excluded.technical_exception,
  ...
  values (selected_pair.study_id, caller_id,
    case when p_status = 'pending' then 'review.cleared' when p_technical_exception then 'review.accepted_with_exception' else 'review.' || p_status::text end,
...
comment on function public.save_expert_review(uuid, public.review_status, text, text, boolean) is
  'Reviewer decisions are pending, accepted, accepted with a documented technical exception, or rejected. Invalid pairs require an exception or rejection.';
