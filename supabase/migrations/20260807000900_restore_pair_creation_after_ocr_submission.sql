-- The OCR submission override must retain the normal second-submission pair
-- creation step. Reconcile already-ready assignments at the same time.
...
declare caller_id uuid := auth.uid(); selected_assignment public.assignments; selected_slot public.assignment_testers; selected_submission public.submissions; selected_protocol public.protocols; missing_evidence text[]; submitted_count integer;
begin
...
do $$ declare assignment_row record; begin
  for assignment_row in select a.id from public.assignments a where a.status = 'ready_for_validation' and not exists (select 1 from public.matched_pairs pair where pair.assignment_id = a.id) loop
    perform private.create_pending_pair(assignment_row.id, null);
  end loop;
end $$;
