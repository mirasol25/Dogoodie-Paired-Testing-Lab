...
begin
  ...
  if selected_assignment.status <> 'in_progress' or selected_slot.status <> 'in_progress' then
  ...
  where public.submissions.status = 'draft'
...
revoke all on function public.save_submission_draft(uuid, numeric, timestamptz, numeric, numeric, text, text, text, text, text, integer, text) from public;
grant execute on function public.save_submission_draft(uuid, numeric, timestamptz, numeric, numeric, text, text, text, text, text, integer, text) to authenticated;
